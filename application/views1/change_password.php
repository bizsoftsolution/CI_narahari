               ...
               <form name="add" method="POST" action="<?php echo  base_url();?>ChangePassword/pwd_update/<?php echo $this->session->userdata('user_id'); ?>" class="form-horizontal">
                    ...
                    <div class="form-body">
						<div class="form-group">
							...
							<div class="col-md-5">
								<?php 
								$data = $this->db->get_where('user', array("user_id"=>$this->session->userdata('user_id')))->result();
								foreach($data as $val)
								{
								?>
								<input name="old_password" placeholder="Old Password" class="form-control" type="text" value="<?php echo $val->password; ?>" disabled>
